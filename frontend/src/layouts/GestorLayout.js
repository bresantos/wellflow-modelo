...
import { IconHome, IconUsers, IconChartBar, IconWind, IconAlertCircle, IconBell, IconLogout, IconMenu2, IconX } from '@tabler/icons-react';
...
const GestorLayout = () => {
  ...
  const [teamAverage, setTeamAverage] = useState({ professional: 0, personal: 0 });

  const menuItems = [
    { icon: IconHome, path: '/gestor', label: 'Home' },
    { icon: IconUsers, path: '/gestor/employees', label: 'Funcionários' },
    { icon: IconChartBar, path: '/gestor/reports', label: 'Relatórios' },
    { icon: IconWind, path: '/gestor/environment', label: 'Ambiente' },
    { icon: IconAlertCircle, path: '/gestor/actions', label: 'Ações' }
  ];

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [notifsRes, avgRes] = await Promise.all([
          axios.get(`${API}/notifications`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`${API}/stats/team-average`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
        setNotifications(notifsRes.data);
        setTeamAverage(avgRes.data);
      } catch (error) {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: '#f5f2e7' }}>
      {/* Sidebar Desktop */}
      <div className="hidden md:flex flex-col items-center py-6 px-2 fixed left-0 top-0 h-screen z-40" style={{ width: '60px', background: 'white', borderRight: '1px solid rgba(178, 63, 230, 0.1)' }}>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="hidden md:flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-inter" style={{ color: '#2e2e2e' }}>Estresse Profissional (Média)</span>
                  <div className="w-32 h-2 rounded-full" style={{ background: 'rgba(178, 63, 230, 0.1)' }}>
                    <div className="h-full rounded-full" style={{ width: `${(teamAverage.professional / 5) * 100}%`, background: '#B23FE6' }}></div>
                  </div>
                  <span className="text-xs font-bold" style={{ color: '#B23FE6' }} data-testid="header-team-professional-stress">{teamAverage.professional}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-inter" style={{ color: '#2e2e2e' }}>Estresse Pessoal (Média)</span>
                  <div className="w-32 h-2 rounded-full" style={{ background: 'rgba(178, 63, 230, 0.1)' }}>
                    <div className="h-full rounded-full" style={{ width: `${(teamAverage.personal / 5) * 100}%`, background: 'rgba(178, 63, 230, 0.6)' }}></div>
                  </div>
                  <span className="text-xs font-bold" style={{ color: 'rgba(178, 63, 230, 0.6)' }} data-testid="header-team-personal-stress">{teamAverage.personal}</span>
                </div>
              </div>
